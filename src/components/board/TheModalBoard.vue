  ...
  <div class="modal">
    <div class="modal__container">
      ...
      <form @submit.prevent="submitBoard">
        ...
        <input type="text" v-model="newBoard.name" placeholder="Введите заголовок доски" />
        <textarea v-model="newBoard.description" placeholder="Введите описание доски"></textarea>
...
export default {
  ...
  methods: {
    ...
    submitBoard() {
      if (this.isFormValid) {
        this.$store
          .dispatch('addBoard', { ...this.newBoard })
          .then(() => {
            this.resetForm();
            this.closeModal();
          })
